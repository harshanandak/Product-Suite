...
from security import Actor, actor_from_credentials, decode_neon_access_token
...
def test_actor_from_credentials_rejects_hosted_claims_missing_email(monkeypatch):
    monkeypatch.setattr(
    ...
    assert getattr(exc_info.value, "detail", None) == "Token payload is missing email"
...
def test_auth_provider_redesign_schema_stores_hashed_invite_tokens_only():
    migration_file = REPO_ROOT / "backend" / "alembic" / "versions" / "0004_auth_provider_redesign.py"
    content = migration_file.read_text(encoding="utf-8")

    assert "token_hash TEXT NOT NULL UNIQUE" in content
    assert " invite_token " not in content
    assert " invite_token_" not in content
