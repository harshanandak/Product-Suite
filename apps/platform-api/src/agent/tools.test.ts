...
describe('buildTools (ToolRegistry)', () => {
  ...
  it('propose_create writes ONLY a proposal, stamped with agent provenance', async () => {
    ...
    const tools = buildTools(sql, { tenantIds: ['t_1'], userId: 'u_1', runId: 'run_1' })
    ...
    // and everything is stamped with the run id for the decision corpus.
  ...
  it('list_work_items scopes by ctx.tenantIds and returns compact fields only', async () => {
    ...
    const tools = buildTools(sql, { tenantIds: ['t_1', 't_2'], userId: 'u_1', runId: 'run_1' })
    ...
    // Never writes; reads are tenant-scoped by the caller's resolved orgs.
