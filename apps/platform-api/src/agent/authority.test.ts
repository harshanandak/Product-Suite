...
describe('compareByAuthority', () => {
  it('higher tier first; ties → scope specificity → event-time recency', () => {
    const t1: AuthorityRankable = { tier: 1, scopeType: 'org', eventTime: '2026-01-01' }
    const t3: AuthorityRankable = { tier: 3, scopeType: 'work_item', eventTime: '2026-07-01' }
    expect(compareByAuthority(t1, t3)).toBeLessThan(0) // t1 first
    const a: AuthorityRankable = { tier: 3, scopeType: 'project', eventTime: '2026-01-01' }
    const b: AuthorityRankable = { tier: 3, scopeType: 'org', eventTime: '2026-01-01' }
    expect(compareByAuthority(a, b)).toBeLessThan(0) // project (more specific) first
  })
})
describe('annotateByAuthority', () => {
  it('annotates a lower-tier chunk highly similar to a higher-tier memory', () => {
    const mem: AnnotatableItem = { id: 'm1', tier: 1, title: 'Use Postgres', embedding: [1, 0, 0] }
    const chunk: AnnotatableItem = { id: 'c1', tier: 3, title: 'DB choice', embedding: [0.99, 0.14, 0] }
    const out = annotateByAuthority([chunk, mem])
  ...
  it('does NOT annotate below the threshold', () => {
    const mem: AnnotatableItem = { id: 'm1', tier: 1, title: 'X', embedding: [1, 0, 0] }
    const chunk: AnnotatableItem = { id: 'c1', tier: 3, title: 'Y', embedding: [0, 1, 0] }
    expect(annotateByAuthority([chunk, mem]).find((x) => x.id === 'c1')!.annotation).toBeUndefined()
  ...
  it('does NOT annotate against a non-memory referent (a T4 meeting vs a T3 chunk stays unannotated)', () => {
    ...
    const chunk: AnnotatableItem = { id: 'c1', tier: 3, title: 'DB choice', embedding: [1, 0, 0] }
    const meeting: AnnotatableItem = { id: 'x1', tier: 4, title: 'Standup aside', embedding: [0.99, 0.14, 0] }
    const out = annotateByAuthority([meeting, chunk])
  ...
  it('annotates a T1 memory that defers to a more-authoritative T0 pinned rule', () => {
    const rule: AnnotatableItem = { id: 'r1', tier: 0, title: 'Always encrypt PII', embedding: [1, 0, 0] }
    const mem: AnnotatableItem = { id: 'm1', tier: 1, title: 'Store SSNs', embedding: [0.99, 0.14, 0] }
    const out = annotateByAuthority([mem, rule])
