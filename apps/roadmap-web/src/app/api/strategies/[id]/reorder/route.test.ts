...
function req(body: unknown) {
  return new NextRequest('http://localhost/api/strategies/strat-1/reorder', {
    method: 'POST',
    body: JSON.stringify(body),
  })
}

describe('POST /api/strategies/[id]/reorder auth', () => {
  beforeEach(() => {
  ...
  it('returns 401 when there are no canonical auth claims', async () => {
    ...
    const res = await POST(req({ sort_order: 0 }), PARAMS)
  ...
  it('returns 403 for non-members', async () => {
    ...
    createClient.mockResolvedValue(
      makeClient({
        product_strategies: { data: { id: 'strat-1', team_id: 'team-1', type: 'objective', parent_id: null } },
