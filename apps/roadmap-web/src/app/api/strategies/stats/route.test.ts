...
describe('GET /api/strategies/stats auth', () => {
  beforeEach(() => {
  ...
  it('returns 400 when team_id is missing', async () => {
    ...
    const res = await GET(new NextRequest('http://localhost/api/strategies/stats'))
  ...
  it('returns 401 when there are no canonical auth claims', async () => {
    ...
    const res = await GET(new NextRequest('http://localhost/api/strategies/stats?team_id=team-1'))
