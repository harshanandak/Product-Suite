...
const { getKnowledgeGraph } = vi.hoisted(() => ({ getKnowledgeGraph: vi.fn() }))

vi.mock('@/lib/auth/get-auth-claims', () => ({ getAuthClaims }))
vi.mock('@/lib/supabase/server', () => ({ createClient }))
vi.mock('@/lib/ai/compression', () => ({ getKnowledgeGraph }))

import { GET } from './route'

const URL = 'http://localhost/api/knowledge/graph'
...
describe('GET /api/knowledge/graph auth', () => {
  beforeEach(() => {
