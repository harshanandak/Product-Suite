...
function getReq() {
  return new NextRequest('http://localhost/api/workspaces/ws-1/integrations')
}
...
describe('workspaces/[id]/integrations route auth', () => {
  beforeEach(() => {
  ...
  it('POST returns 401 when there are no claims', async () => {
    createClient.mockResolvedValue(client({}))
    getAuthClaims.mockResolvedValue(null)
    const res = await POST(postReq({ integrationId: 'i1' }), ctx())
    expect(res.status).toBe(401)
  })

  it('POST returns 400 when integrationId is missing', async () => {
    getAuthClaims.mockResolvedValue(CLAIMS)
    createClient.mockResolvedValue(
      client({
        workspaces: { data: { id: 'ws-1', team_id: 't1' } },
  ...
  it('POST returns 404 when the integration is not found', async () => {
    getAuthClaims.mockResolvedValue(CLAIMS)
    createClient.mockResolvedValue(
      client({
        workspaces: { data: { id: 'ws-1', team_id: 't1' } },
        team_members: { data: { team_id: 't1' } },
        organization_integrations: { data: null },
      })
    )
    const res = await POST(postReq({ integrationId: 'i1' }), ctx())
    expect(res.status).toBe(404)
