...
const STATS_URL = 'http://localhost/api/insights/stats?team_id=team-1'

// Membership lookup: team_members -> select -> eq(team) -> eq(user) -> single
...
describe('GET /api/insights/stats auth', () => {
  beforeEach(() => {
  ...
  it('returns 400 when team_id is missing', async () => {
    ...
    const res = await GET(new NextRequest('http://localhost/api/insights/stats'))
  ...
  it('returns 401 when there are no canonical auth claims', async () => {
    ...
    const res = await GET(new NextRequest(STATS_URL))
