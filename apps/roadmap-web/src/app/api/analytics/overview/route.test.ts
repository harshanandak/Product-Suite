...
const URL_WITH_PARAMS =
  'http://localhost/api/analytics/overview?workspace_id=ws-1&team_id=team-1'
...
describe('GET /api/analytics/overview auth', () => {
  beforeEach(() => {
  ...
  it('returns 400 when workspace_id/team_id are missing', async () => {
    ...
    const res = await GET(new NextRequest('http://localhost/api/analytics/overview'))
