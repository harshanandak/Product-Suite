...
function jsonReq(method: string, body: unknown) {
  return new Request('http://localhost/api/feedback/f-1', {
    method,
...
describe('GET /api/feedback/[id] auth', () => {
  beforeEach(() => {
  ...
  it('returns 401 when there are no canonical auth claims', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/feedback/f-1'), PARAMS)
...
describe('PATCH /api/feedback/[id] auth', () => {
  beforeEach(() => {
    getAuthClaims.mockReset()
    createClient.mockReset()
  })
  ...
  it('returns 401 when there are no canonical auth claims', async () => {
    ...
    const res = await PATCH(jsonReq('PATCH', {}), PARAMS)

    expect(res.status).toBe(401)
  })
})

describe('DELETE /api/feedback/[id] auth', () => {
  beforeEach(() => {
    getAuthClaims.mockReset()
    createClient.mockReset()
  })
  ...
  it('returns 401 when there are no canonical auth claims', async () => {
    ...
    const res = await DELETE(new Request('http://localhost/api/feedback/f-1', { method: 'DELETE' }), PARAMS)
