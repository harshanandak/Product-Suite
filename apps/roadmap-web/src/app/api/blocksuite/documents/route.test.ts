...
const URL = 'http://localhost/api/blocksuite/documents'
const validCreate = { workspaceId: 'ws-1', documentType: 'canvas', title: 'My Doc' }
...
describe('POST /api/blocksuite/documents', () => {
  beforeEach(() => {
  ...
  it('returns 401 when there are no canonical auth claims', async () => {
    ...
    const res = await POST(postReq(validCreate))
  ...
  it('returns 403 when claims subject is not a team member', async () => {
    ...
    createClient.mockResolvedValue(
      makeClient({
        workspaces: { data: { id: 'ws-1', team_id: 't1' } },
...
describe('GET /api/blocksuite/documents', () => {
  beforeEach(() => {
    getAuthClaims.mockReset()
    createClient.mockReset()
  })
  ...
  it('returns 401 when there are no canonical auth claims', async () => {
    createClient.mockResolvedValue(makeClient({}))
    getAuthClaims.mockResolvedValue(null)
    const res = await GET(new NextRequest(`${URL}?workspaceId=ws-1`))
    expect(res.status).toBe(401)
  })

  it('returns 400 for a missing/invalid workspaceId', async () => {
    getAuthClaims.mockResolvedValue(CLAIMS)
    createClient.mockResolvedValue(makeClient({}))
