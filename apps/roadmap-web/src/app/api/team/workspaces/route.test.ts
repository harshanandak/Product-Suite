...
function membershipClient(membership: { id: string; role: string } | null) {
...
describe('GET /api/team/workspaces auth', () => {
  beforeEach(() => {
  ...
  it('returns 401 when there are no canonical auth claims', async () => {
    ...
    const res = await GET(new NextRequest('http://localhost/api/team/workspaces?team_id=team-1'))
  ...
  it('returns 400 when team_id is missing', async () => {
    ...
    createClient.mockResolvedValue({ from: vi.fn() })
    const res = await GET(new NextRequest('http://localhost/api/team/workspaces'))
