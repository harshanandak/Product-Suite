...
the shared database either. The seed is atomic for the same reason: the meeting and
its action item are inserted in one transaction, so a seed that fails part way
through leaves nothing for `afterEach` to miss.
