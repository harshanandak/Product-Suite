...
- Hosting: Cloudflare (shell assets + Hono platform API); meeting-api and agent workers on Railway.
- Data access: backend-mediated only — the browser talks to platform APIs, never directly to the database.
- **Full stack matrix (decided 2026-06-12, six-agent verified research):** see [`docs/plans/tech-stack-evaluation-2026-06-12.md`](docs/plans/tech-stack-evaluation-2026-06-12.md). Headlines: **Postgres is the only database, Durable Objects the only realtime transport, Railway the only containers; everything else is a library.** Canvas + graph = React Flow (+ perfect-freehand ink); docs = TipTap core + y-prosemirror; **BlockSuite is exiting — do not use it in any new code; React Flow + TipTap core is the canvas stack, and `patches/` is removed with it**; realtime = partyserver/Durable Objects (Hocuspocus-on-Railway and Supabase Realtime both retire); agent runtime = **AI SDK v6 + OpenRouter provider (model-flexible — hard requirement; Claude/OpenAI agent SDKs rejected for model lock-in)**, same code in Hono Worker and Railway workers; durable spine = pgmq + pg_cron + proposals, run resumability persisted in `agent.runs`; MCP = official GitHub/Meta remote servers + FastMCP + Cloudflare McpAgent; meetings = AssemblyAI + Gemini Flash extraction + pgvector RAG (~$0.20/meeting-hr); search/files/notifications/analytics = Postgres-first + R2/Uppy/Resend/Sentry free tiers; UI = TanStack Table v8 + Virtual, dnd-kit, custom Gantt, cmdk, react-virtuoso, recharts.
