...
### Security
- Retire archived Roadmap Web from active dependency installs, builds and delivery checks, removing its Next.js runtime while preserving source and historical migration evidence (Forge issue `cee01acc-7055-4684-8bd5-7a8b7fdc3eb9`)
...
### Changed
- First-push validation now uses the verified Git pre-push range to run affected checks, while unsupported updates and uncertain ranges fail closed (Forge issue `1944f7bd-fcaf-4131-b58a-3ddc1f8b753b`)
...
### Fixed
- Branch-lease concurrency tests now observe persisted waiter registration, recognize signal-terminated workers, retain child ownership until termination, and preserve temporary lease files when bounded cleanup fails (PR #187, Forge issue `b6c7f336-8eea-4e07-9321-21d8c8b4501b`)
