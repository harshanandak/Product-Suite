...
9. Ensure Python `3.13` is available for the Meeting API toolchain.
10. Run `bun run install:meeting-api` for the Python backend dependencies.
11. Run `bun run validate` from the repo root.
...
- `.github/workflows/repo-tooling-ci.yml` watches shared package and service paths and runs the repo tooling guard plus shared package and agent-core tests.
