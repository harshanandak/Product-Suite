...
> **Re-verified 2026-06-12 (second adversarial pass, focused on parallel multi-agent + write-executing tools + durable approval gates): KEEP AI SDK v6 + OpenRouter.** Findings:
> - **AI SDK v6** has everything the requirement list needs today: GA Agent abstraction, documented **subagents/agents-as-tools fan-out pattern**, one-flag `needsApproval` HITL, full MCP client, healthy OpenRouter provider (v2.9.x). Persistence stays BYO — which our Queues + `agent.runs`/`proposals` spine already implements with our semantics.
> - **Named upgrade path: AI SDK v7's `WorkflowAgent`** (pre-release) adds durable state + approval flows surviving restarts, but currently rides Vercel's Workflow DevKit — **re-evaluate at v7 GA only if portable to Cloudflare Containers**; if so it drop-in replaces the hand-rolled cursor. This, not LangGraph, is the natural evolution.
> - **LangGraph correction:** the JS *library* is MIT (only Server/Platform is ELv2); PostgresSaver + `interrupt()` is best-in-class durable HITL, supervisor/swarm packages exist, OpenRouter works via baseURL. Remains fallback #2 — switching would rewrite the loop around graph/checkpoint semantics and add a streaming adapter to reach `useChat`, buying little we haven't already built.
> - **Mastra:** relicensed Apache 2.0, $35M funded, Postgres suspend/resume — credible fallback #1, **but** has open bugs precisely on the agent↔suspendable-workflow approval path (mastra#11015, #11283), which is exactly our gate. Not today.
> - **TanStack AI is real** (MIT, `@tanstack/ai` v0.28.x, daily releases): a provider-agnostic UI/chat SDK with experimental orchestration — competes at the `useChat` layer, far too pre-1.0 for the agent spine; also recently weathered an npm supply-chain incident. **Watch-list**: could one day replace AI SDK's UI hooks for TanStack-ecosystem coherence, never the runtime today.
> - OpenAI Agents SDK is model-agnostic only *through* an AI SDK adapter (use AI SDK directly); Pydantic AI stays the Python-side option; Inngest AgentKit couples to Inngest; VoltAgent too young; Agno/CrewAI are Python.
> - **OpenRouter reliability noted:** ~3 outages/8 months, no SLA — mitigated by the AI SDK provider seam (direct Anthropic/Google/Moonshot keys are drop-in if an outage window matters).
>
> Re-verification sources: https://github.com/TanStack/ai · https://www.npmjs.com/package/@tanstack/ai · https://tanstack.com/blog/tanstack-ai-orchestration · https://snyk.io/blog/tanstack-npm-packages-compromised/ · https://ai-sdk.dev/docs/agents/subagents · https://ai-sdk.dev/v7/docs/agents/workflow-agent · https://github.com/vercel/workflow · https://www.npmjs.com/package/@openrouter/ai-sdk-provider · https://rvernica.github.io/2026/03/langchain-license · https://www.npmjs.com/package/@langchain/langgraph-checkpoint-postgres · https://docs.langchain.com/oss/javascript/langgraph/persistence · https://www.npmjs.com/package/@langchain/langgraph-supervisor · https://mastra.ai/blog/apache-license · https://mastra.ai/blog/series-a · https://mastra.ai/docs/workflows/suspend-and-resume · https://github.com/mastra-ai/mastra/issues/11015 · https://openai.github.io/openai-agents-js/extensions/ai-sdk/ · https://ai.pydantic.dev/durable_execution/dbos/ · https://agentkit.inngest.com/ · https://ofox.ai/blog/is-openrouter-reliable-honest-review-2026/
| Durable spine | **Revised 2026-06-12 (cost doctrine — Cloudflare-first for primitives): a jobs seam.** SaaS = **Cloudflare Queues (HTTP-pull consumers from Railway workers) + Cron Triggers** — usage-priced, keeps Postgres compute light; self-host = **pgmq + pg_cron** (zero extra services in compose). `agent.runs`/`proposals` tables stay in Postgres either way. Upgrade path unchanged: Trigger.dev v4 when orchestration outgrows the seam; Inngest second option |
...
### 3a. Agent execution tiers (added 2026-06-12): Worker → Dynamic Workers → Containers

**Cloudflare Dynamic Workers (Worker Loader API, open beta on Workers Paid; $0.002/unique worker/day from May 26 2026, waived in beta)** join the stack as the middle execution tier: throwaway isolates spawned at runtime in <5ms, 100× faster / 1/10 the memory of containers. Uses: (1) **Code Mode for the MCP gateway** — for multi-tool data-heavy operations the agent writes a script against loaded tool APIs, executed in a sandboxed isolate; only results return to context (composes with search_tools; ~80% token savings claimed on data processing); (2) **instant fan-out of short-lived sub-agents** (extract/check/summarize bursts); (3) **sandboxing agent-authored code** (automations, dashboard queries) in isolates with no ambient credentials. **Boundary:** long durable runs (supervisor loops, pause-on-approval resumability) stay on Cloudflare Containers — isolate memory/lifecycle isn't built for hour-long runs. The ladder: regular Worker (light sync) → dynamic isolates (instant short work + Code Mode) → Containers (long durable runs). Sources: https://blog.cloudflare.com/dynamic-workers/ · https://developers.cloudflare.com/dynamic-workers/ · https://developers.cloudflare.com/dynamic-workers/pricing/ · https://www.infoq.com/news/2026/04/cloudflare-dynamic-workers-beta/ · https://blog.cloudflare.com/durable-object-facets-dynamic-workers/

### 3b. MCP gateway and connector dashboards (added 2026-06-12, seventh research pass)

**MCP gateway — BUILD a thin router on our stack (~1–2 engineer-weeks).** Requirement: agents get `search_tools`/`load_tool`/`call_tool` meta-tools; schemas lazy-load per call. Landscape verified: Docker MCP Gateway (MIT) has the closest "Dynamic MCP" semantics but is Docker-engine/session-scoped, not multi-tenant; MetaMCP (MIT) and MCPJungle (MPL) are static allow-lists without search; LiteLLM's MCP layer is allow-lists; Cloudflare MCP Server Portals is an enterprise governance product; Composio/Pipedream put credentials in their cloud (Pipedream closed to Workday, Jan 2026); Anthropic's Tool Search / `defer_loading` (85% token cut) is **Anthropic-API-only — unusable via AI SDK + OpenRouter**, which settles build-vs-buy: the search layer must live in our router to be model-portable. Build: registry + encrypted per-user creds + FTS-indexed `tool_schemas` cache + three native AI SDK tools + credential-injecting Hono proxy. **Update (2026-06-12): Nous Research's Hermes Agent (MIT) is the open, model-portable reference — its BM25 Tool Search replaces MCP schemas with three bridge tools and auto-activates only when deferred schemas would exceed ~10% of model context (their measurement: 5 servers/34 tools ≈ 45k tokens/turn, ~22k pure schema overhead). Lift its BM25 scorer, bridge-tool schemas, and auto-mode threshold into our router; Hermes stays a single-deployment agent framework, so the multi-tenant registry/creds/proxy build is still ours.**
...
**§3b update (2026-06-12) — catalog + scope + containers:** the gateway ships with an **MCP catalog synced from the official MCP Registry** (registry.modelcontextprotocol.io, ~9,600 servers May 2026; PulseMCP ~15.9k, Smithery ~7.3k, mcp.so ~20k) in three tiers (Verified curated / Indexed full-registry / Custom URL), remote-transport-first — we host nothing in Tiers 1–2; stdio-only servers get a hosted runner fleet (cloud-only) on **Cloudflare Containers**, which also replaces Railway for both container workloads (scale-to-zero, per-10ms billing, 4 GiB/½ vCPU instances — fine for I/O-bound agent loops and FastAPI; Railway = named fallback). Connections carry `scope: user|workspace` — user-scoped by default (personal identity, usable only in that user's runs), workspace-scoped admin-only with explicit shared-data badge; resolution: user connection → workspace connection → prompt; audit records the connection used.

**§3b sources:** https://registry.modelcontextprotocol.io/ · https://github.com/modelcontextprotocol/registry · https://nordicapis.com/7-mcp-registries-worth-checking-out/ · https://developers.cloudflare.com/containers/pricing/ · https://developers.cloudflare.com/containers/platform-details/limits/ · https://github.com/NousResearch/hermes-agent · https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-search · https://www.marktechpost.com/2026/05/29/hermes-agent-ships-tool-search-for-mcp-anthropic-evals-show-49-to-74-accuracy-gain-on-opus-4/ · https://github.com/docker/mcp-gateway · https://docs.docker.com/ai/mcp-catalog-and-toolkit/dynamic-mcp/ · https://github.com/metatool-ai/metamcp · https://github.com/mcpjungle/MCPJungle · https://docs.litellm.ai/docs/mcp · https://developers.cloudflare.com/cloudflare-one/access-controls/ai-controls/mcp-portals/ · https://blog.cloudflare.com/enterprise-mcp/ · https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-search-tool · https://modelcontextprotocol.io/specification/draft/server/tools · https://composio.dev/pricing · https://pipedream.com/connect · https://docs.airbyte.com/community/licenses/license-faq · https://docs.airbyte.com/integrations/sources/facebook-marketing · https://docs.airbyte.com/integrations/sources/google-analytics-data-api · https://www.cloudquery.io/blog/how_to_combine_google_and_facebook_ads_data · https://grafana.com/grafana/plugins/blackcowmoo-googleanalytics-datasource/ · https://github.com/airbytehq/airbyte/issues/76483
