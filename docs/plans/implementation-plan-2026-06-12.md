...
**Open decision this PR exists to settle — the F1 fork:** PR #25 created an in-Next-app platform shell, while F1 as written calls for a fresh `apps/platform-web` on Vite + TanStack Router (DESIGN §10, repo topology above). Two coherent paths:
...
Recommendation: **(A)** — the whole rebuild rationale (DESIGN §10, tech-stack eval §7/§7a) is that Next-on-Vercel is an exit, not a destination; building lane UI on it now compounds migration debt. But this is a founder/architecture call — resolve it here before F1 starts so no lane builds on the wrong foundation.
