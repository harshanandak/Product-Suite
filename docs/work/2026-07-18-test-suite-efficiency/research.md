...
- **`isolate: false` (`--no-isolate`).** Biggest single lever for suites that clean up their own state (typical for `node`-env suites). Re-uses one environment across files instead of a fresh one per file. It can be scoped per-`projects` — isolated for the few side-effecty files, non-isolated for the rest. Not available in `vmThreads`. Source: https://vitest.dev/guide/improving-performance (Test Isolation)
