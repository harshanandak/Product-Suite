...
**Requires Durable Objects:** the delta *fan-out*. A plain Worker can hold a connected WebSocket for the life of a request, but it has no durable identity or shared state across clients, so it cannot be the place many clients rendezvous to receive broadcasts. Durable Objects exist precisely for that — durable coordination, shared state, and scalable fan-out — "Each Durable Object has a globally-unique name, which allows you to send requests to a specific object from anywhere… coordinate between multiple clients," with **WebSocket Hibernation** so idle sockets do not bill for wall-clock compute ([Cloudflare Durable Objects](https://developers.cloudflare.com/durable-objects/)). Shape: one DO per workspace (or per Linear-style *sync group*), holding sockets, broadcasting deltas, and — critically — *not* being the source of truth. Postgres stays authoritative; the DO is a pub/sub relay plus a `lastSyncId` watermark.
...
**Recommendation:** do not impose a UI depth limit; impose two invariants. (1) **Single parent FK** — OpenProject enforces this ("No, this is not possible"); Vikunja notably does *not* (parent is a relation kind, so multiple parents are allowed). A single parent keeps the tree a tree and rollups unambiguous. (2) **Cycle prevention on write** — this, plus a cycle-safe query, is what actually guarantees a recursive CTE terminates; the single-parent FK alone does not (a chain can still loop without it). Then note the operational trap: OpenProject derives parent progress from children and **ignores manually-entered work on parents** — the moment you have a hierarchy, rollup fields must be computed, not stored, or they will disagree.
