...
**We already have a second level.** Our `tasks` table is a flat per-work-item checklist. Adding `parent_id` on work items with a **1-level cap** gives us: work item → child work item → checklist task. That is Jira's shape exactly (Story → Sub-task → checklist), and it is one tier richer than Asana's own recommendation. The Jira-community "use checklists as the extra level below sub-task" workaround is us, natively.

**What depth costs us specifically:**

- **Cycle prevention.** At depth ≤ 1 the invariant is `parent.parent_id IS NULL` — a single-row check, no recursion. At depth 3 every move must walk ancestors, and two concurrent moves that are each individually valid can jointly create a cycle. Preventing that needs serializable retries or a materialized `path`/`ltree` with a unique-prefix invariant. **Depth 1 makes the race impossible.**
  > **Correction, added post-hoc:** this section originally asserted the Neon HTTP driver has no transactions, making serializable retries "awkward." That is false — `sql.transaction()` supports isolation levels over HTTP, and WebSockets give full interactive transactions on Workers. The depth-1 recommendation stands on the product evidence in §1–§3; this particular argument is weaker than written.
