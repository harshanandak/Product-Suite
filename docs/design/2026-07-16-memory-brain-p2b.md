...
- **First-run freeze:** the hash is computed ONLY on a thread's first run. `mintRun` first looks up the thread's earliest `chat` run's `memory_holdout` and REUSES it when present; only a brand-new thread (or a thread-less run) computes fresh. This makes a thread's cohort immutable once committed — a later `MEMORY_HOLDOUT_RATE` change can never flip an in-flight conversation mid-stream (which would contaminate the whole-thread counterfactual).
...
- Per cohort: `applied` = count(status='applied'), `edited` = count(status='applied' AND edited_payload IS NOT NULL), `editRate = edited/applied`; plus `rejected` = count(status='rejected') and `rejectRate = rejected/(applied+rejected)`; plus `threads` = count(DISTINCT thread_id) among applied — the independent-unit count the clustering guard reads.
...
- **Confidence gate (the honest guard):** a closed-form **Newcombe (Wilson-score hybrid) two-proportion 95% CI** on `delta` (no library, ~15 lines) — chosen over the normal-approx Wald interval, which undercovers badly at small n with edit rates near 0 or 1 (exactly the small-sample/extreme-p regime this gate exists to protect against). `verdict`:
  - `insufficient` — either cohort below `MIN_SAMPLE` (default 20), OR either cohort below `MIN_THREADS` distinct applied threads (default 5 — the clustering guard, below), OR the CI straddles 0 (indistinguishable from noise).
...
- **Metric:** fixture proposals across cohorts → correct per-cohort editRate; **signed** `savedEdits = delta × treated volume` (a negative delta yields a negative savedEdits, NOT 0); `verdict='insufficient'` below MIN_SAMPLE, below `MIN_THREADS` distinct applied threads in either cohort (clustering guard), and when the CI straddles 0; `verdict='helps'` only when ciLow>0; `verdict='hurts'` when ciHigh<0; **reflection/`kind<>'chat'` runs are excluded**; a material rejectRate divergence downgrades to `insufficient`.
...
- **Thread-clustered CI** — v1's two-proportion CI treats proposals as independent (mildly optimistic given within-thread clustering). The interim conservatism is the `MIN_THREADS` gate (≥5 distinct applied threads per cohort) in `decideVerdict`; a clustered/bootstrap CI is the tracked refinement that will replace it.
...
- **Very low-thread orgs may sit entirely in one cohort** (e.g. a tiny org with zero holdout threads) → `verdict='insufficient'` indefinitely. This is honest by construction — the surface says "measuring — not enough data yet," never a fabricated number. We deliberately do NOT revert to per-run to "fix" coverage (that reintroduces within-thread spillover); if low-volume coverage becomes a product concern, the lever is the deferred org-level rate config, not a change to the causal unit. (Note: a thread's assignment is FROZEN to its first run — `mintRun` reuses the thread's earliest `chat`-run `memory_holdout` — so a later `RATE` change can NOT flip an existing thread; it only affects threads whose first run comes after the change.)

## 8. Defaults (change on request)
`MEMORY_HOLDOUT_RATE = 0.10` (env-overridable, 0 disables) · deterministic per-thread (FNV-1a) assignment, FROZEN to the thread's first run · `MIN_SAMPLE = 20` applied proposals per cohort · `MIN_THREADS = 5` distinct applied threads per cohort (interim clustering guard) · Newcombe (Wilson-score) two-proportion 95% CI as the honest gate · `rejectRate` divergence threshold 0.10 · window default 30 days (on `decided_at`).
